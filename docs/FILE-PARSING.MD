...
The parser extracts: **Title**, **Year**, **Edition**, **Resolution**, **Source** (persisted as `source_type`), **Season/Episode** (for TV), **Artist/Album/Track** (for music).
...
|---|---|---|
| `[tmdbid=12345]` or `[tmdbid-12345]` | `Movie [tmdbid=550].mkv` | TMDB ID 550 |
| `[imdbid=tt1234567]` or `[imdbid-tt1234567]` | `Movie [imdbid=tt0133093].mkv` | IMDB ID tt0133093 |
| `[tvdbid=12345]` or `[tvdbid-12345]` | `Show [tvdbid=73255].mkv` | TVDB ID 73255 |

Both `=` and `-` separators are accepted. When a TMDB ID is found, the scanner performs a **direct TMDB lookup** by ID, bypassing both the cache server and fuzzy title matching entirely. The brackets are stripped from the title before further parsing.
...
**NFO types supported:**
- **Movie NFOs** (`<movie>` root element) — title, tagline, plot, year, runtime, MPAA, country, trailer URL, ratings, genres, studios, cast, directors, writers, artwork references, provider IDs (IMDB, TMDB, TVDB), source type, HDR format, dynamic range, custom notes
...
| Type | Names checked |
|---|---|
| Poster | `poster`, `movie-poster`, `cover`, `folder`, `<filename>-poster` |
| Backdrop | `backdrop`, `fanart`, `background`, `<filename>-fanart`, `<filename>-backdrop` |
| Logo | `logo`, `clearlogo`, `<filename>-logo` |
| Banner | `banner`, `<filename>-banner` |
...
- **HDR format detection**: Identifies Dolby Vision (via DOVI RPU side data), HDR10 (PQ transfer + BT.2020 primaries), HLG (arib-std-b67 transfer), or generic PQ
- **Dynamic range classification**: HDR (any HDR format detected) or SDR (default)
- **Audio format detection**: Enhanced audio format string that detects spatial audio features on top of the base codec. Identifies Dolby Atmos (on TrueHD or EAC3) via side data, profile, and channel count heuristics. Identifies DTS:X on DTS-HD MA via profile detection. Returns display-friendly names (e.g., "TrueHD Atmos", "EAC3 Atmos", "DTS-HD MA DTS:X", "FLAC", "AAC")

FFprobe values **override** any filename-parsed values for resolution and container. Source type from filename parsing (e.g., bluray, web-dl, hdtv) is persisted to the `source_type` field on the media item.
...
**Confidence scoring**: 0.6 minimum threshold, +0.20 boost for exact year match, -0.10 penalty for off-by-one year, -0.40 penalty for year mismatch by 2+ years
...
- TMDB keywords stored as JSON on the media item
- Mood tags auto-assigned from keyword mappings (see [RECOMMENDATIONS.MD](RECOMMENDATIONS.MD) for mood categories)
...
### Video Codecs
`x264`, `x265`, `h264`, `h265`, `h.264`, `h.265`, `hevc`, `avc`, `divx`, `divx5`, `xvid`, `10bit`, `8bit`, `hi10p`, `hi10`, `av1`, `vp9`, `mpeg4`

### Audio Codecs
`aac`, `ac3`, `ac-3`, `dts`, `dts-hd`, `dtshd`, `dts-x`, `truehd`, `atmos`, `flac`, `mp3`, `ogg`, `vorbis`, `opus`, `eac3`, `dd5.1`, `dd2.0`, `5.1`, `7.1`, `2.0`, `5.1ch`, `7.1ch`

### Resolution
`480p`, `480i`, `576p`, `576i`, `720p`, `720i`, `1080p`, `1080i`, `2160p`, `4k`, `uhd`, `ultrahd`, `hd`, `sd`

### Source
`bluray`, `blu-ray`, `bdrip`, `brrip`, `bdremux`, `bdrc`, `hdrip`, `hddvd`, `hddvdrip`, `dvd`, `dvdrip`, `dvdscr`, `dvdscreener`, `webrip`, `web-dl`, `webdl`, `hdtv`, `pdtv`, `dsr`, `dsrip`, `tvrip`, `cam`, `screener`, `scr`, `tc`, `telecine`, `ts`, `telesync`, `retail`, `ppv`, `stv`

### Container Tokens
`mkv`, `mp4`, `avi` (when appearing as tokens in the filename, not as the file extension)

### Release / Misc
`remux`, `proper`, `repack`, `rerip`, `internal`, `limited`, `custom`, `extended`, `unrated`, `remastered`, `theatrical`, `special.edition`, `multi`, `multisubs`, `dubbed`, `subbed`, `subs`, `sub`, `ws`, `fs`, `fragment`, `xxx`, `nfo`, `read.nfo`, `readnfo`, `nfofix`, `1cd`–`4cd`, `cd1`–`cd4`
...
| 3. Filename Parsing | Extract title, year, edition, resolution, source type, season/episode |
...
| 7. FFprobe Analysis | Extract technical metadata (duration, codec, resolution, HDR format, dynamic range) |
